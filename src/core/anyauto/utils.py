...
PINYIN_POOL = [
    ...
    "xi", "xia", "xian", "xiang", "xiao", "xie", "xin", "jing", "xiong", "xiu", "xu", "xuan", "xue", "xun",
...
def generate_pinyin_prefix(min_len=2, max_len=3):
    """随机生成 2-3 个拼音组合作为前缀"""
    num = random.randint(min_len, max_len)
    pinyins = [random.choice(PINYIN_POOL) for _ in range(num)]
    # 加入少量随机数字
    suffix = "".join(random.choices(string.digits, k=random.randint(2, 4)))
    return "".join(pinyins) + suffix
