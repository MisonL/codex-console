(function () {
  ...
  function isBlockedInviter(item) {
    const status = String(item?.status || "")
      ...
      .toLowerCase();
    const source = String(
      item?.manager_verify_source || item?.verify_source || "",
    ).trim();
  ...
  function extractTokenFields(accessToken) {
    ...
    const planRaw = String(auth.chatgpt_plan_type || "")
      .trim()
      .toLowerCase();
    let plan = planRaw;
  ...
  class TeamManageConsole {
    constructor() {
      this.els = {
        ...
        btnCloseTeamImportModal: document.getElementById(
          "btnCloseTeamImportModal",
        ),
        btnTeamImportSingleTab: document.getElementById(
          "btnTeamImportSingleTab",
        ),
        ...
        teamImportRefreshToken: document.getElementById(
          "teamImportRefreshToken",
        ),
        teamImportSessionToken: document.getElementById(
          "teamImportSessionToken",
        ),
        ...
        btnCloseTeamMemberModal: document.getElementById(
          "btnCloseTeamMemberModal",
        ),
        ...
        teamInvitedMembersBody: document.getElementById(
          "teamInvitedMembersBody",
        ),
      };
    ...
    bindEvents() {
      ...
      this.els.teamStatusFilter?.addEventListener("change", () =>
        this.applyFilters(),
      );
      this.els.teamSearchInput?.addEventListener("input", () =>
        this.applyFilters(),
      );
      this.els.btnReloadTeamConsole?.addEventListener("click", () =>
        this.loadConsole(true, true),
      );
      this.els.btnImportTeam?.addEventListener("click", () =>
        this.openImportModal(),
      );
      this.els.btnCloseTeamImportModal?.addEventListener("click", () =>
        this.closeImportModal(),
      );
      this.els.teamImportModal?.addEventListener("click", (e) => {
        if (e.target === this.els.teamImportModal) this.closeImportModal();
      });
      this.els.btnTeamImportSingleTab?.addEventListener("click", () =>
        this.setImportMode("single"),
      );
      this.els.btnTeamImportBatchTab?.addEventListener("click", () =>
        this.setImportMode("batch"),
      );
      this.els.btnSubmitTeamImport?.addEventListener("click", () =>
        this.submitTeamImport(),
      );
      ...
      this.els.teamColumnMenu?.addEventListener("change", (e) =>
        this.onColumnToggle(e),
      );

      this.els.teamSelectAll?.addEventListener("change", () => {
        const checked = !!this.els.teamSelectAll.checked;
        this.els.teamTableBody
          ?.querySelectorAll("input[data-row-select]")
          .forEach((el) => {
            el.checked = checked;
          });
      });

      this.els.teamTableBody?.addEventListener("click", (e) =>
        this.handleTableAction(e),
      );

      this.els.btnCloseTeamMemberModal?.addEventListener("click", () =>
        this.closeMemberModal(),
      );
      this.els.teamMemberModal?.addEventListener("click", (e) => {
        if (e.target === this.els.teamMemberModal) this.closeMemberModal();
      });
      this.els.btnInviteMember?.addEventListener("click", () =>
        this.inviteMember(),
      );
      this.els.btnReloadTeamMembers?.addEventListener("click", () =>
        this.loadMembers(true),
      );
      this.els.teamJoinedMembersBody?.addEventListener("click", (e) =>
        this.handleMemberTableAction(e, "joined"),
      );
      this.els.teamInvitedMembersBody?.addEventListener("click", (e) =>
        this.handleMemberTableAction(e, "invited"),
      );

      document.addEventListener("click", (e) => {
        if (!this.els.teamColumnMenu || !this.els.btnToggleColumnMenu) return;
        if (
          !this.els.teamColumnMenu.contains(e.target) &&
          !this.els.btnToggleColumnMenu.contains(e.target)
        ) {
    ...
    readHiddenCols() {
      try {
        ...
        return Array.isArray(parsed)
          ? parsed.filter((x) => COLUMN_KEYS.includes(x))
          : [];
      } catch (_e) {
        return [];
      }
        ...
        if (existing) {
          ...
          nextMap.set(
            ...
            this.normalizeRow({
              ...existing,
              ...incoming,
              current_members: Number.isFinite(currentMembers)
                ? currentMembers
                : 0,
              max_members:
        ...
        if (existing) {
          nextMap.set(
            ...
            this.normalizeRow({
              ...
              max_members:
                Number.isFinite(Number(incoming.max_members)) &&
                Number(incoming.max_members) > 0
                  ? Number(incoming.max_members)
                  : Number(existing.max_members || DEFAULT_TEAM_MAX_MEMBERS),
              member_ratio:
                incoming.member_ratio || existing.member_ratio || "0/5",
            }),
          );
        } else {
    ...
    updateRowMemberStats(accountId, joinedCount) {
      ...
      const maxMembers =
        Number.isFinite(maxMembersRaw) && maxMembersRaw > 0
          ? maxMembersRaw
          : DEFAULT_TEAM_MAX_MEMBERS;
    ...
    onColumnToggle(event) {
      ...
      if (!target || !target.matches('input[type="checkbox"][data-col-toggle]'))
        return;
    ...
    applyColumnVisibility() {
      ...
      this.els.teamColumnMenu
        ?.querySelectorAll('input[type="checkbox"][data-col-toggle]')
        .forEach((el) => {
    ...
    normalizeRow(row) {
      ...
      const maxRaw = Number(
        row.max_members || row.maxMembers || DEFAULT_TEAM_MAX_MEMBERS,
      );
      const max =
      ...
      const current = Number.isFinite(currentRaw)
        ? Math.max(0, Math.min(currentRaw, max))
        : 0;
      return {
        id: Number(row.id || 0),
    ...
    async loadRowsFromInviterPool(force = false, seq = null) {
      try {
        ...
        const inviterData = await api.get(
          `/auto-team/inviter-accounts${query}`,
          {
            timeoutMs: 10000,
            retry: 0,
            priority: "high",
        ...
        const inviters = (
          Array.isArray(inviterData.accounts) ? inviterData.accounts : []
        ).filter((item) => !isBlockedInviter(item));
        if (!inviters.length) {
          return false;
        }
        this.syncRowsFromInviters(inviters, {
          pruneMissing: false,
          applyStats: false,
        });
        return true;
        ...
        if (
          ...
          error?.cancelReason === "request_replaced"
        ) {
          return false;
        }
        return false;
    ...
    async refreshConsoleRemote(seq, rowsBeforeLoad, options = {}) {
      ...
      try {
        ...
        const remoteSynced = this.syncRowsFromConsole(consoleRows, {
          applyStats: false,
        });
        ...
        if (
          ...
          if (withToast)
            toast.warning(
              "当前无可用 Team 管理账号，已保留历史 Team 列表并标记为过期",
            );
          return;
        }
        ...
        if (fallbackOk) {
          ...
          if (statusCode === 404 || /not found/i.test(msg)) {
            ...
            if (withToast)
              toast.success("已切换邀请池模式（team-console 不可用）");
            return;
          }
          if (withToast)
            toast.warning(`team-console 读取失败，已回退邀请池：${msg}`);
          return;
        }
        if (rowsBeforeLoad.length) {
          ...
          if (withToast)
            toast.warning(
              `team-console 读取失败，已保留当前列表（待校验）：${msg}`,
            );
          return;
        }
    ...
    async loadConsole(withToast, force = false) {
      ...
      const rowsBeforeLoad = Array.isArray(this.rows)
        ? this.rows.map((x) => this.normalizeRow(x))
        : [];
      ...
      if (force) {
        try {
          ...
          if (!manualSynced && rowsBeforeLoad.length) {
            ...
            if (withToast)
              toast.warning(
                "本地未匹配到可用管理账号，已保留历史 Team 列表并标记为过期",
              );
        ...
        void this.refreshConsoleRemote(seq, rowsBeforeLoad, {
          force: true,
          withToast: false,
        });
        return;
      ...
      try {
        loading.show(this.els.btnReloadTeamConsole, "刷新中...");
        await this.refreshConsoleRemote(seq, rowsBeforeLoad, {
          force: false,
          withToast,
        });
      } finally {
    ...
    setStats(total, available) {
      if (this.els.teamStatTotal)
        this.els.teamStatTotal.textContent = String(total || 0);
      if (this.els.teamStatAvailable)
        this.els.teamStatAvailable.textContent = String(available || 0);
    }
    ...
    applyFilters() {
      const status = String(this.els.teamStatusFilter?.value || "")
        .trim()
        .toLowerCase();
      const keyword = String(this.els.teamSearchInput?.value || "")
        .trim()
        .toLowerCase();
    ...
    renderRows() {
      ...
      if (!this.loaded) {
        this.els.teamTableBody.innerHTML =
          '<tr><td colspan="9" class="empty-tip">首次不自动加载，请点击“刷新”读取 Team 列表</td></tr>';
        return;
      }
      if (!this.filteredRows.length) {
        this.els.teamTableBody.innerHTML =
          '<tr><td colspan="9" class="empty-tip">暂无 Team 管理账号</td></tr>';
        return;
      }
      this.els.teamTableBody.innerHTML = this.filteredRows
        .map(
          (row) => `
                <tr>
    ...
    setImportHint(text, isError = false) {
      ...
      this.els.teamImportModalHint.style.color = isError
        ? "var(--danger-color)"
        : "";
    }
    ...
    setImportMode(mode) {
      ...
      this.els.btnTeamImportSingleTab?.classList.toggle(
        "active",
        next === "single",
      );
      this.els.btnTeamImportBatchTab?.classList.toggle(
        "active",
        next === "batch",
      );
      this.els.teamImportSinglePanel?.classList.toggle(
        "active",
        next === "single",
      );
      this.els.teamImportBatchPanel?.classList.toggle(
        "active",
        next === "batch",
      );
    }
    ...
    buildImportItemFromRaw(rawItem) {
      const accessToken = String(
        rawItem.access_token || rawItem.accessToken || "",
      ).trim();
      const refreshToken = String(
        rawItem.refresh_token || rawItem.refreshToken || "",
      ).trim();
      const sessionToken = String(
        rawItem.session_token || rawItem.sessionToken || "",
      ).trim();
      const clientIdInput = String(
        rawItem.client_id || rawItem.clientId || "",
      ).trim();
      const emailInput = String(rawItem.email || "")
        .trim()
        .toLowerCase();
      const accountIdInput = String(
        rawItem.account_id || rawItem.accountId || "",
      ).trim();
    ...
    async submitTeamImport() {
      try {
        ...
        if (this.teamImportMode === "single") {
          ...
          rawItems = this.parseBatchImportText(
            this.els.teamImportBatchText?.value || "",
          );
        }
    ...
    async refreshOne(accountId) {
      const data = await api.post(
        `/auto-team/team-accounts/${accountId}/refresh`,
        {},
        {
          timeoutMs: 15000,
          retry: 0,
    ...
    async openMemberModal(accountId, email) {
      ...
      if (this.els.teamMemberModalSub)
        this.els.teamMemberModalSub.textContent = this.memberAccountEmail;
      if (this.els.teamMemberInviteEmail)
        this.els.teamMemberInviteEmail.value = "";
      const cacheKey = String(accountId);
      const cached =
        this.membersCache && typeof this.membersCache === "object"
          ? this.membersCache[cacheKey]
          : null;
      const cachedJoined = Array.isArray(cached?.joined_members)
        ? cached.joined_members
        : Array.isArray(cached?.joined)
          ? cached.joined
          : [];
      const cachedInvited = Array.isArray(cached?.invited_members)
        ? cached.invited_members
        : Array.isArray(cached?.invited)
          ? cached.invited
          : [];
      if (cached) {
        ...
        if (this.els.teamJoinedMembersBody)
          this.els.teamJoinedMembersBody.innerHTML =
            '<tr><td colspan="4" class="empty-tip">加载中...</td></tr>';
        if (this.els.teamInvitedMembersBody)
          this.els.teamInvitedMembersBody.innerHTML =
            '<tr><td colspan="4" class="empty-tip">加载中...</td></tr>';
      }
    ...
    async loadMembers(withToast, options = {}) {
      ...
      try {
        ...
        const data = await api.get(
          `/auto-team/team-accounts/${accountId}/members`,
          {
            timeoutMs: 15000,
            retry: 0,
            priority: "high",
        ...
        const joined = Array.isArray(data.joined_members)
          ? data.joined_members
          : [];
        const invited = Array.isArray(data.invited_members)
        ...
        if (preserveExisting && hasCached) {
          if (this.els.teamMemberModalHint)
            this.els.teamMemberModalHint.textContent = `读取失败，已显示缓存: ${msg}`;
          if (withToast) toast.warning(`读取失败，已显示缓存：${msg}`);
          return;
        }
        if (this.els.teamJoinedMembersBody)
          this.els.teamJoinedMembersBody.innerHTML = `<tr><td colspan="4" class="empty-tip">${esc(msg)}</td></tr>`;
        if (this.els.teamInvitedMembersBody)
          this.els.teamInvitedMembersBody.innerHTML =
            '<tr><td colspan="4" class="empty-tip">-</td></tr>';
        if (this.els.teamMemberModalHint)
          this.els.teamMemberModalHint.textContent = `读取失败: ${msg}`;
        if (withToast) toast.error(msg);
      } finally {
    ...
    renderJoined(rows) {
      ...
      if (!rows.length) {
        this.els.teamJoinedMembersBody.innerHTML =
          '<tr><td colspan="4" class="empty-tip">暂无已加入成员</td></tr>';
        return;
      }
      this.els.teamJoinedMembersBody.innerHTML = rows
        .map(
          (item) => `
                <tr>
    ...
    renderInvited(rows) {
      ...
      if (!rows.length) {
        this.els.teamInvitedMembersBody.innerHTML =
          '<tr><td colspan="4" class="empty-tip">暂无邀请中成员</td></tr>';
        return;
      }
      this.els.teamInvitedMembersBody.innerHTML = rows
        .map(
          (item) => `
                <tr>
                    <td>${esc(item.email || "-")}</td>
    ...
    async inviteMember() {
      const accountId = Number(this.memberAccountId || 0);
      if (!accountId) return;
      const email = String(this.els.teamMemberInviteEmail?.value || "")
        .trim()
        .toLowerCase();
      const emailRe = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
      ...
      try {
        ...
        const data = await api.post(
          `/auto-team/team-accounts/${accountId}/members/invite`,
          { email },
        );
    ...
    async handleMemberTableAction(event, type) {
      ...
      try {
        ...
        if (type === "invited" && action === "revoke") {
          ...
          if (!email) return;
          await api.post(
            `/auto-team/team-accounts/${accountId}/members/revoke`,
            { email },
          );
          toast.success("邀请已撤回");
          ...
          if (!userId) return;
          await api.post(
            `/auto-team/team-accounts/${accountId}/members/remove`,
            { user_id: userId },
          );
          toast.success("成员已移除");
        } else {
