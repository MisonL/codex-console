  ...
  class AutoTeamPage {
    constructor() {
      this.els = {
        ...
        targetModalSelectedInfo: document.getElementById(
          "targetModalSelectedInfo",
        ),
        ...
        manualInviterSelectedInfo: document.getElementById(
          "manualInviterSelectedInfo",
        ),
        btnCloseManualInviterModal: document.getElementById(
          "btnCloseManualInviterModal",
        ),
        btnManualInviterSelectAll: document.getElementById(
          "btnManualInviterSelectAll",
        ),
        btnManualInviterClearAll: document.getElementById(
          "btnManualInviterClearAll",
        ),
        btnSubmitManualInviter: document.getElementById(
          "btnSubmitManualInviter",
        ),
    ...
    bindEvents() {
      this.els.tabInvite?.addEventListener("click", () =>
        this.switchTab("invite"),
      );
      this.els.tabManage?.addEventListener("click", () =>
        this.switchTab("manage"),
      );
      this.els.btnPickTargetEmail?.addEventListener("click", () =>
        this.openTargetModal(),
      );
      this.els.btnReloadInviterList?.addEventListener("click", () =>
        this.loadInviterAccounts(true, this.els.btnReloadInviterList, true),
      );
      this.els.btnManualPullInviter?.addEventListener("click", () =>
        this.openManualInviterModal(),
      );
      this.els.btnCloseTargetModal?.addEventListener("click", () =>
        this.closeTargetModal(),
      );
      this.els.btnTargetSelectAll?.addEventListener("click", () =>
        this.selectVisibleTargets(),
      );
      this.els.btnTargetClearAll?.addEventListener("click", () =>
        this.clearSelectedTargets(),
      );
      this.els.btnAddSelectedTargets?.addEventListener("click", () =>
        this.addSelectedTargetsToInput(),
      );
      this.els.targetModalSearch?.addEventListener("input", () =>
      ...
      this.els.targetModalList?.addEventListener("change", (e) => {
        const el = e.target;
        if (!el || !el.matches('input[type="checkbox"][data-target-id]'))
          return;
      ...
      this.els.btnCloseManualInviterModal?.addEventListener("click", () =>
        this.closeManualInviterModal(),
      );
      this.els.btnManualInviterSelectAll?.addEventListener("click", () =>
        this.selectVisibleManualInviters(),
      );
      this.els.btnManualInviterClearAll?.addEventListener("click", () =>
        this.clearSelectedManualInviters(),
      );
      this.els.btnSubmitManualInviter?.addEventListener("click", () =>
        this.submitManualInviterSelection(),
      );
      this.els.manualInviterSearch?.addEventListener("input", () =>
        this.renderManualInviterList(),
      );
      this.els.manualInviterList?.addEventListener("change", (e) => {
        const el = e.target;
        if (!el || !el.matches('input[type="checkbox"][data-inviter-id]'))
          return;
      ...
      this.els.btnReloadAccounts?.addEventListener("click", () =>
        this.loadInviterAccounts(true, this.els.btnReloadAccounts, true),
      );
      this.els.btnReloadTeamGroups?.addEventListener("click", () =>
        this.loadTeamGroups(true, true),
      );
    ...
    async bootstrap() {
      this.log(
        "team页面已加载，已关闭首次自动刷新。请手动点击“刷新”加载 Team 管理账号列表。",
      );
      ...
      const cachedManagers = Array.isArray(cachedGroups?.managers)
        ? cachedGroups.managers
        : [];
      const cachedMembers = Array.isArray(cachedGroups?.members)
        ? cachedGroups.members
        : [];
      ...
      if (cachedManagers.length > 0 || cachedMembers.length > 0) {
        ...
        this.log(
          `已载入本地缓存分类: 母号=${this.teamManagers.length} 子号=${this.teamMembers.length}`,
        );
      } else {
    ...
    pruneInviterAccounts(list, stage = "") {
      ...
      const sorted = [...filtered].sort((a, b) => {
        const aRole = String(a?.role_tag || "")
          .trim()
          .toLowerCase();
        const bRole = String(b?.role_tag || "")
          .trim()
          .toLowerCase();
    ...
    setReloadButtonLoading(button, isLoading) {
      const btn =
        button || this.els.btnReloadInviterList || this.els.btnReloadAccounts;
      if (!btn) return;
      if (isLoading) {
        if (!btn.dataset.originalLabel) {
          btn.dataset.originalLabel =
            String(btn.textContent || "").trim() || "刷新";
        }
        btn.disabled = true;
    ...
    renderInviters(list) {
      ...
      if (!Array.isArray(list) || list.length === 0) {
        if (this.manualLoadMode && !this.inviterLoaded) {
          this.els.inviterList.innerHTML =
            '<div class="empty-tip">首次不自动加载，请点击“刷新”读取 Team 管理账号。</div>';
          return;
        }
        this.els.inviterList.innerHTML =
          '<div class="empty-tip">暂无可用 Team 管理账号（需 team + token + workspace）。</div>';
        return;
      }

      this.els.inviterList.innerHTML = list
        .map((item) => {
    ...
    fillSelect(list) {
      ...
      this.els.inviterAccount.innerHTML =
        '<option value="">自动选择（默认第一个可用 Team 管理账号）</option>';
      list.forEach((item) => {
    ...
    async loadTeamGroups(withToast, force = false) {
      try {
        ...
        const hasExistingGroups =
          (Array.isArray(this.teamManagers) && this.teamManagers.length > 0) ||
        ...
        if (
          ...
          this.renderTeamGroupList(
            this.els.managerList,
            this.teamManagers,
            true,
          );
          this.renderTeamGroupList(
        ...
        this.renderTeamGroupList(this.els.memberList, this.teamMembers, false);
        this.log(
          `team分类加载完成: 母号=${managers.length} 子号=${members.length}`,
        );
        if (withToast) {
          toast.success(
            `已刷新：母号 ${managers.length}，子号 ${members.length}`,
          );
        }
      } catch (error) {
        const msg = safeError(error);
    ...
    async loadTargetAccounts(withToast) {
      try {
        ...
        this.renderTargetModalList();
        this.log(
          `目标邮箱候选账号已加载: ${accounts.length} 个（邀请锁定 ${lockedTotal}）`,
        );
        if (withToast) {
    ...
    getFilteredTargetAccounts() {
      const q = String(
        fp.normalizeValue(this.els.targetModalSearch?.value) || "",
      ).toLowerCase();
    ...
    renderTargetModalList() {
      ...
      if (!list.length) {
        container.innerHTML =
          '<div class="empty-tip">暂无可选账号（仅 free 且非红色状态）</div>';
        this.updateTargetSelectedInfo();
        return;
      }
    ...
    addSelectedTargetsToInput() {
      const selectedItems = this.targetAccounts.filter((x) =>
        this.selectedTargetIds.has(String(x.id)),
      );
    ...
    async openManualInviterModal() {
      ...
      if (
        Array.isArray(this.manualInviterCandidates) &&
        this.manualInviterCandidates.length
      ) {
        this.renderManualInviterList();
      } else if (this.els.manualInviterList) {
        this.els.manualInviterList.innerHTML =
          '<div class="empty-tip">加载候选账号中...</div>';
        this.updateManualInviterSelectedInfo();
      }
    ...
    async loadManualInviterCandidates(withToast = false) {
      try {
        ...
        this.manualInviterCandidates = Array.isArray(data.accounts)
          ? data.accounts
          : [];
        this.renderManualInviterList();
        this.log(
          `手动拉入候选已加载: ${this.manualInviterCandidates.length} 个`,
        );
        if (withToast) {
    ...
    getFilteredManualInviterCandidates() {
      const q = String(
        fp.normalizeValue(this.els.manualInviterSearch?.value) || "",
      ).toLowerCase();
    ...
    renderManualInviterList() {
      ...
      if (!list.length) {
        container.innerHTML =
          '<div class="empty-tip">暂无可拉入账号（仅母号/普通）</div>';
        this.updateManualInviterSelectedInfo();
        return;
      }
      container.innerHTML = list
        .map((item) => {
          const id = String(item.id || "");
          const checked = this.selectedManualInviterIds.has(id)
            ? "checked"
            : "";
    ...
    selectVisibleManualInviters() {
      const list = this.getFilteredManualInviterCandidates();
      list.forEach((item) =>
        this.selectedManualInviterIds.add(String(item.id)),
      );
      this.renderManualInviterList();
    }
    ...
    async submitManualInviterSelection() {
      const ids = [...this.selectedManualInviterIds]
        .map((x) => Number(x))
      ...
      try {
        ...
        this.log(
          `手动拉入完成: added=${added} skipped=${skipped} invalid=${invalid}`,
        );
        toast.success(
          `拉入完成：新增 ${added}，跳过 ${skipped}，无效 ${invalid}`,
        );
        this.closeManualInviterModal();
        this.selectedManualInviterIds.clear();
        await this.loadInviterAccounts(
          false,
          this.els.btnReloadInviterList,
          true,
        );
    ...
    async loadInviterAccounts(withToast, loadingBtn = null, force = false) {
      ...
      try {
        ...
        if (accounts.length > 0) {
          this.log(
            `读取可用 Team 邀请账号完成: ${accounts.length} 个（自动按管理号入池）`,
          );
        } else {
          this.log(
            "读取可用 Team 邀请账号完成: 0 个（已按最新规则清空不符合账号）",
          );
        }
        if (withToast) {
          if (accounts.length > 0) {
        ...
        if (abortLike) {
          try {
            ...
            const retryRaw = Array.isArray(retryData.accounts)
              ? retryData.accounts
              : [];
            const retryAccounts = this.pruneInviterAccounts(
              retryRaw,
              "中断重试",
            );
        ...
        if (this.inviterAccounts.length > 0) {
          const kept = this.pruneInviterAccounts(
            this.inviterAccounts,
            "失败保留",
          );
    ...
    async runSilentPrecheck(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
          ...
          if (invalidCount > 0) {
            this.log(
              `自动预检提示: 发现无效邮箱 ${invalidCount} 个，执行时将自动跳过。`,
            );
          }
          this.log(
            `自动预检成功: inviter=${data.inviter?.email || "-"} | ${tip}`,
          );
          return data;
        } catch (error) {
          lastError = error;
          ...
          if (retryable && attempt < maxAttempts) {
            this.log(
              `自动预检网络波动，第 ${attempt}/${maxAttempts} 次重试中...`,
            );
        ...
        const resultType =
          failedCount > 0
            ? successCount > 0
              ...
              : "error"
            : "success";
        this.setResult(resultType, "自动邀请结果", summary, {
        ...
        if (successfulEmails.size) {
          ...
          this.selectedTargetIds = new Set(
            [...this.selectedTargetIds].filter((id) => {
              const item = this.targetAccounts.find(
