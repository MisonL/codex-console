...
const AUTO_REFRESH_SCOPE_STORAGE_KEY =
  "accounts_overview_auto_refresh_scope_v1";
...
const overviewApi = {
  ...
  attachCard(id) {
    return api.post(
      `/accounts/overview/cards/${id}/attach`,
      {},
      {
        timeoutMs: 15000,
        retry: 0,
...
const overviewState = {
  ...
  cardAutoRefreshScope:
    storage.get(AUTO_REFRESH_SCOPE_STORAGE_KEY, "stale_failed") ||
    "stale_failed",
};
...
function getCardsForRefresh(mode = "visible") {
  const source =
    Array.isArray(overviewState.filteredCards) &&
    overviewState.filteredCards.length
      ? overviewState.filteredCards
      : overviewState.cards;
  ...
  return source.filter((item) => {
    ...
    const hourlyUnknown =
      String(item?.hourly_quota?.status || "").toLowerCase() === "unknown";
    const weeklyUnknown =
      String(item?.weekly_quota?.status || "").toLowerCase() === "unknown";
...
async function refreshSelectedOrAll(
  force = true,
  silent = false,
  options = {},
) {
  ...
  try {
    ...
    if (!silent) {
      if (status === "completed") {
        toast.success(
          `刷新完成：成功 ${result.success_count || 0}，失败 ${result.failed_count || 0}`,
        );
        ...
        toast.error(
          `刷新失败: ${finalTask?.error || finalTask?.message || "未知错误"}`,
        );
      }
    }
  } catch (error) {
...
async function removeSelectedAccounts() {
  ...
  const ok = await confirm(
    `确认从卡片列表删除 ${ids.length} 个账号吗？（不会删除账号管理数据）`,
    "批量删除卡片",
  );
  if (!ok) return;
...
function bindEvents() {
  ...
  if (cardRefreshScope) {
    const nextScope = ["stale_failed", "visible"].includes(
  ...
  const toolbarRefreshFailedBtn = document.getElementById(
    "card-refresh-failed-btn",
  );
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  initResults.forEach((item, index) => {
    if (item.status === "rejected") {
      const target =
        index === 0 ? "总览统计" : index === 1 ? "卡片列表" : "可选账号";
