(function () {
  ...
  function sanitizeRecord(record) {
    ...
    const status = VALID_STATUSES.has(record?.status)
    ...
    const expiresAt =
      record?.expires_at && Number.isFinite(Date.parse(record.expires_at))
        ? record.expires_at
        : null;
    const usedAt =
      record?.used_at && Number.isFinite(Date.parse(record.used_at))
        ? record.used_at
        : null;
    return {
      code,
  ...
  function renderSupplierOptions() {
    ...
    const hasEmptySupplier = state.codes.some(
      (item) => !normalizeSupplier(item.supplier),
    );
    ...
    if (
      ...
      importSupplierSelect.tagName.toLowerCase() === "select"
    ) {
      const currentImport = String(importSupplierSelect.value || "").trim();
      const importOptions = ['<option value="">未设置供应商</option>'].concat(
        list.map(
          (name) =>
            `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`,
        ),
      ...
      if (
        currentImport &&
        Array.from(importSupplierSelect.options).some(
          (option) => option.value === currentImport,
        )
      ) {
  ...
  function renderTable() {
    ...
    if (!pageRows.length) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="empty-row">暂无匹配数据。</td></tr>';
    } else {
    ...
    const allChecked =
      state.pageCodes.length > 0 &&
      state.pageCodes.every((code) => state.selectedCodes.has(code));
    document.getElementById("select-all-codes").checked = allChecked;
    document.getElementById("btn-delete-selected").disabled =
      state.selectedCodes.size === 0;
  }
  ...
  function setActiveTab(tab) {
    ...
    document
      .getElementById("pool-tab-redeem")
      .classList.toggle("active", state.activeTab === "redeem");
    document
      .getElementById("pool-tab-credit")
      .classList.toggle("active", state.activeTab === "credit");
    document
      .getElementById("panel-redeem")
      .classList.toggle("active", state.activeTab === "redeem");
    document
      .getElementById("panel-credit")
      .classList.toggle("active", state.activeTab === "credit");
  }
  ...
  function setStatusFilter(status) {
    ...
    document.querySelectorAll(".status-chip").forEach((btn) => {
      btn.classList.toggle(
        "active",
        (btn.dataset.status || "") === state.statusFilter,
      );
    });
    render();
  }
  ...
  function openEditModal(code) {
    ...
    document.getElementById("edit-supplier-input").value = normalizeSupplier(
      target.supplier,
    );
    document.getElementById("edit-status-select").value = VALID_STATUSES.has(
      target.status,
    )
      ? target.status
      : "unused";
    document.getElementById("edit-used-email").value = String(
      target.used_by_email || "",
    ).trim();
    document.getElementById("edit-expires-at").value =
      toLocalDateTimeInputValue(target.expires_at);
  ...
  function handleEditConfirm() {
    ...
    const nextStatus = String(
      document.getElementById("edit-status-select").value || "unused",
    );
    ...
    const nextSupplier = normalizeSupplier(
      document.getElementById("edit-supplier-input").value,
    );
    const nextEmail = String(
      document.getElementById("edit-used-email").value || "",
    ).trim();
    const nextExpiresIso = parseLocalDateTimeToIso(
      document.getElementById("edit-expires-at").value,
    );
    const rawExpires = String(
      document.getElementById("edit-expires-at").value || "",
    ).trim();
  ...
  function handleImportConfirm() {
    ...
    if (parsed.invalidCount > 0)
      fragments.push(`格式错误 ${parsed.invalidCount} 条`);
  ...
  function bindEvents() {
    document
      .getElementById("pool-tab-redeem")
      .addEventListener("click", () => setActiveTab("redeem"));
    document
      .getElementById("pool-tab-credit")
      .addEventListener("click", () => setActiveTab("credit"));
    ...
    document
      ...
      .addEventListener("change", (event) => {
        state.supplierFilter = String(
          fp.normalizeValue(event.target.value) || "",
        );
        state.page = 1;
        render();
      });
    ...
    document
      .getElementById("redeem-search-btn")
      .addEventListener("click", applySearch);
    document
      .getElementById("redeem-search")
    ...
    document
      .getElementById("redeem-codes-body")
      .addEventListener("change", (event) => {
    ...
    document
      .getElementById("select-all-codes")
      .addEventListener("change", (event) => {
    ...
    document
      .getElementById("btn-import-codes")
      .addEventListener("click", openImportModal);
    document
      .getElementById("btn-export-codes")
      .addEventListener("click", exportCurrentRows);
    document
      .getElementById("btn-delete-selected")
      .addEventListener("click", deleteSelectedCodes);
    ...
    document
      ...
      .addEventListener("click", closeEditModal);
    document
      .getElementById("edit-modal-confirm")
