...
async function watchPaymentOpTask(
  taskId,
  onUpdate,
...
async function saveManualSessionToken() {
  ...
  try {
    ...
    if (data?.success) {
      toast.success(
        `Session Token 已保存（len=${data?.session_token_len || 0}）`,
      );
      await runSessionDiagnostic();
      return;
    }
...
function parseCardText(rawText) {
  ...
  // 卡号
  const cardKeyCandidates = ["卡号", "card number", "card", "card_number"];
    ...
    for (const line of lines) {
      if (/(账单地址|billing\s*address)/i.test(line)) {
        addressCandidate = line
          .replace(/^.*?(账单地址|billing\s*address)\s*[:：]?\s*/i, "")
          .trim();
...
function buildParsedSummary(parsed) {
  ...
  if (parsed.exp_month || parsed.exp_year) {
    parts.push(
      `有效期: ${formatExpiryInput(parsed.exp_month, parsed.exp_year) || "--/--"}`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      setRandomBillingHint(
        `来源: ${sourceLabel} | ${city}, ${state}, ${postal} | 外部失败: ${fallbackReason}`,
        "error",
      );
      toast.warning(`外部地址源不可用，已切换本地兜底：${fallbackReason}`);
    } else {
      setRandomBillingHint(
        `来源: ${sourceLabel} | ${city}, ${state}, ${postal}`,
        "success",
      );
...
function parseBatchBillingText() {
  ...
  setParseResult(
    `批量识别成功：共 ${billingBatchProfiles.length} 条`,
    "success",
  );
}
...
function getBindMode() {
  ...
  const mode =
    String(modeSelect?.value || "semi_auto")
      .trim()
...
async function openGptOfficialLogin() {
  ...
  try {
    ...
    window.open(loginUrl, "_blank", "noopener,noreferrer");
    toast.warning(
      `打开无痕失败，已在当前浏览器打开: ${formatErrorMessage(error)}`,
    );
  }
}
...
function appendVendorLogs(logs) {
  ...
  const lines = logs.map(
    (item) => `[${item?.time || "--:--:--"}] ${item?.message || ""}`,
  );
  const existing = String(logBox.textContent || "").trim();
  logBox.textContent = existing
    ? `${existing}\n${lines.join("\n")}`
    : lines.join("\n");
...
function updateVendorRuntimeHint(progressPayload = {}, status = "running") {
  ...
  const elapsed = Math.max(
    0,
    Number(
      progressPayload?.elapsed_seconds ??
        vendorProgressState.elapsedSeconds ??
        0,
    ),
  );
  const forceStopAfter = Math.max(
    1,
    Number(
      progressPayload?.force_stop_after_seconds ??
        vendorProgressState.forceStopAfterSeconds ??
        120,
    ),
  );
  const canForceStop =
    Boolean(progressPayload?.can_force_stop) || elapsed >= forceStopAfter;
...
function updateVendorProgressUi(progressPayload, taskId) {
  const progress = Math.max(
    0,
    Math.min(100, Number(progressPayload?.progress || 0)),
  );
...
async function pollVendorProgress(taskId) {
  ...
  try {
    const payload = await api.get(
      `/payment/bind-card/tasks/${taskId}/vendor-progress?cursor=${vendorProgressState.cursor}`,
    );
    const progressPayload = payload?.progress || {};
    const logs = progressPayload?.logs || [];
    appendVendorLogs(logs);
    vendorProgressState.cursor = Number(
      progressPayload?.next_cursor || vendorProgressState.cursor || 0,
    );
    ...
    if (status === 404 && detail.includes("绑卡任务不存在")) {
      appendVendorLogs([
        {
          time: "--:--:--",
          message: "进度服务未命中任务，已停止轮询（可刷新任务列表后重试）",
        },
      ]);
      stopVendorProgressPolling();
      setVendorStopButtonState(false, true);
    ...
    appendVendorLogs([
      {
        time: "--:--:--",
        message: `进度轮询失败: ${formatErrorMessage(error)}`,
      },
    ]);
    vendorProgressState.timer = setTimeout(() => {
      pollVendorProgress(taskId);
...
async function startVendorAutoBind(task, vendorConfig) {
  ...
  appendVendorLogs([
    {
      time: "--:--:--",
      message: `任务 #${taskId} 已创建，开始执行卡商接口订阅...`,
    },
  ]);
  const data = await api.post(
    `/payment/bind-card/tasks/${taskId}/auto-bind-vendor`,
    {
      redeem_code: redeemCode,
      checkout_url:
        String(vendorConfig?.checkout_url || "").trim() || undefined,
  ...
  updateVendorProgressUi(
    data?.progress || { progress: 5, status: "running" },
    taskId,
  );
...
async function retryVendorAutoTask() {
  ...
  const config = {
    ...(vendorProgressState.lastConfig || {}),
    redeem_code: String(
      (vendorProgressState.lastConfig || {}).redeem_code ||
        fallbackConfig.code ||
        "",
    ).trim(),
    api_url: String(
      (vendorProgressState.lastConfig || {}).api_url ||
        fallbackConfig.api_url ||
        "",
    ).trim(),
    api_key: String(
      (vendorProgressState.lastConfig || {}).api_key ||
        fallbackConfig.api_key ||
        "",
    ).trim(),
    checkout_url: String(
      (vendorProgressState.lastConfig || {}).checkout_url || "",
    ).trim(),
  };
...
async function stopVendorAutoTask() {
  ...
  try {
    if (!taskId) {
      const activeStop = await api.post(
        "/payment/bind-card/vendor-stop-active",
        {},
      );
      ...
      updateVendorProgressUi(
        activeStop?.progress || { status: "cancelled", progress: 100 },
        taskId,
      );
      vendorProgressState.cursor = Number(
        activeStop?.progress?.next_cursor || vendorProgressState.cursor || 0,
      );
      stopVendorProgressPolling();
    ...
    for (const path of stopPaths) {
      try {
        ...
        const isRouteNotFound =
          status === 404 && (!detail || detail.toLowerCase() === "not found");
        if (isRouteNotFound) {
    ...
    appendVendorLogs(data?.progress?.logs || []);
    updateVendorProgressUi(
      data?.progress || { status: "cancelled", progress: 100 },
      taskId,
    );
    vendorProgressState.cursor = Number(
      data?.progress?.next_cursor || vendorProgressState.cursor || 0,
    );
    stopVendorProgressPolling();
    toast.success(`任务 #${taskId} 已发送停止指令`);
    await loadBindCardTasks(true);
...
function collectVendorConfig() {
  ...
  const redeemCode = normalizeVendorRedeemCode(
...
function isEfunSupplier(value) {
  ...
  return (
    normalized === CARD_POOL_EFUN_SUPPLIER_KEY || normalized === "efuncard"
  );
}
...
function loadCardPoolRedeemCodes() {
  const parsed = safeJsonParse(
    localStorage.getItem(CARD_POOL_REDEEM_STORAGE_KEY) || "[]",
    [],
  );
...
async function redeemEfunAndFill(showToast = true, overrideConfig = null) {
  ...
  if (month || year)
    setInputValue("card-expiry-input", formatExpiryInput(month, year));
  if (cvc) setInputValue("card-cvc-input", cvc);
  if (!getInputValue("billing-country-input"))
    setInputValue("billing-country-input", "US");
  ...
  if (showToast) {
    toast.success(
      `EFun 开卡成功，已回填卡号 ${String(card.masked || "").trim() || ""}`,
    );
  }
  return data;
}
...
function updateBindModeSpecificUi(mode) {
  ...
  const cardAddressSection = document.getElementById(
    "bind-card-address-section",
  );
  ...
  if (thirdPartyPanel)
    thirdPartyPanel.style.display = bindMode === "third_party" ? "" : "none";
  if (vendorPanel) vendorPanel.style.display = isVendor ? "" : "none";
  if (vendorEfunPanel)
    vendorEfunPanel.style.display = isVendorEfun ? "" : "none";
  if (commonFields) commonFields.style.display = isVendor ? "none" : "";
  if (pasteSection) pasteSection.style.display = isVendorEfun ? "none" : "";
  if (cardAddressSection)
    cardAddressSection.style.display = isVendorEfun ? "none" : "";
...
function restoreBindModeConfig() {
  ...
  if (modeSelect) {
    modeSelect.value = ALLOWED_BIND_MODES.has(savedMode)
      ? savedMode
      : "semi_auto";
  }
  ...
  setInputValue(
    "efun-base-url-input",
    String(
      storage.get(EFUN_BASE_URL_STORAGE_KEY, EFUN_BASE_URL_DEFAULT) ||
        EFUN_BASE_URL_DEFAULT,
    ).trim() || EFUN_BASE_URL_DEFAULT,
  );
  setInputValue(
    "efun-api-key-input",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("vendor-redeem-input")?.addEventListener(
    "input",
    debounce(() => {
      const normalized = normalizeVendorRedeemCode(
        getInputValue("vendor-redeem-input"),
      );
...
async function fetchBindFailStats(accountIds = []) {
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      map.set(
        accountId,
        Number.isFinite(failCount) && failCount > 0 ? failCount : 0,
      );
    });
    return map;
  } catch (_e) {
...
async function loadAccounts() {
  try {
    ...
    const bindFailCountMap = await fetchBindFailStats(
      allAccounts.map((acc) => Number(acc?.id || 0)),
    );
    const toRoleRank = (acc) => {
      const role = String(acc?.role_tag || "")
        .trim()
        .toLowerCase();
    ...
    paymentAccounts = allAccounts
      .filter((acc) => {
        const sub = String(acc?.subscription_type || "")
          .trim()
          .toLowerCase();
        if (sub === "plus" || sub === "team") {
          return false;
        }
        const roleTag = String(acc?.role_tag || "")
          .trim()
          .toLowerCase();
        const accountLabel = String(acc?.account_label || "")
          .trim()
          .toLowerCase();
        const isParent =
          roleTag === "parent" ||
...
async function runLocalAutoBindInBackground(task, bindData) {
  try {
    ...
    if (autoResult?.verified) {
      toast.success(
        `任务 #${task.id} 全自动绑卡完成: ${String(autoResult.subscription_type || "").toUpperCase()}`,
      );
    } else if (autoResult?.paid_confirmed) {
      toast.success(
        `任务 #${task.id} 已确认支付，等待订阅同步（可点“同步订阅”）`,
        7000,
      );
      ...
      toast.warning(
        `任务 #${task.id} 全自动绑卡执行完成，但当前订阅为 ${sub}，请稍后再同步`,
        7000,
      );
    }
  } catch (autoErr) {
    toast.error(
      `任务 #${task.id} 全自动绑卡失败: ${formatErrorMessage(autoErr)}`,
    );
  } finally {
    try {
...
async function createBindCardTask() {
  ...
  if (isVendorEfun) {
    ...
    if (!resolvedCode) {
      renderEfunPoolHint();
      toast.warning(
        "卡商EFun模式需要兑换码：可先在卡池添加供应商=EFun的可用码，或手动填写",
      );
      return;
    }
    ...
    renderEfunPoolHint();
    setEfunResult(
      "已准备就绪：将执行接口流程（强制生成 Checkout -> EFun 开卡 -> bindcard 提交 -> 同步订阅）",
    );
  } else {
    ...
    if (missing.length && bindMode === "semi_auto") {
      toast.warning(
        `绑卡资料未完整：${missing.join("、")}（本次仅创建半自动任务，不会阻断）`,
        5000,
      );
    }
  ...
  payload.auto_open = isVendorEfun
    ? false
    : Boolean(document.getElementById("bind-auto-open")?.checked);
  ...
  try {
    ...
    if (bindMode === "local_auto") {
      ...
      toast.info(
        `任务 #${data.task.id} 已创建，已在后台执行全自动绑卡，可继续修改参数并创建新任务`,
        5000,
      );
      ...
      toast.success(
        `绑卡任务已创建 #${data.task.id}${data.auto_opened ? "，浏览器已打开" : ""}`,
      );
...
async function loadBindCardTasks(silent = false) {
  ...
  const runner = async () => {
    ...
    try {
      ...
      const data = await api.get(
        `/payment/bind-card/tasks?${params.toString()}`,
        {
          timeoutMs: 45000,
          retry: silent ? 0 : 1,
...
async function markBindCardTaskUserAction(taskId) {
  ...
  try {
    ...
    const opTask = await api.post(
      `/payment/bind-card/tasks/${taskId}/mark-user-action/async`,
      {
        timeout_seconds: 180,
        interval_seconds: 10,
      },
    ...
    if (finalStatus === "cancelled" || data?.cancelled) {
      ...
      toast.success(
        `任务 #${taskId} 验证成功: ${String(data.subscription_type || "").toUpperCase()}`,
      );
    } else {
    ...
    if (isRouteNotFound) {
      try {
        const fallbackMark = await api.post(
        ...
        if (fallbackMark?.verified) {
          toast.success(
            `任务 #${taskId} 验证成功: ${String(fallbackMark.subscription_type || "").toUpperCase()}`,
          );
        } else {
          const sub = String(
            fallbackMark?.subscription_type || "free",
          ).toUpperCase();
          toast.warning(
            `任务 #${taskId} 兼容验证完成，但当前仍是 ${sub}`,
            5000,
          );
        }
        await loadBindCardTasks();
        ...
        const isMarkRouteMissing =
          fallbackErr?.response?.status === 404 && detail2 === "not found";
        if (isMarkRouteMissing) {
          try {
            ...
            const sub = String(
              fallback?.subscription_type || "free",
            ).toUpperCase();
            if (sub === "PLUS" || sub === "TEAM") {
              toast.success(`任务 #${taskId} 已通过兼容模式同步成功: ${sub}`);
            } else {
              toast.warning(
                `任务 #${taskId} 兼容同步完成，但当前仍是 ${sub}`,
                5000,
              );
            }
            await loadBindCardTasks();
            ...
            toast.error(
              `验证订阅失败（兼容模式也失败）: ${formatErrorMessage(fallbackSyncErr)}`,
            );
            return;
          }
        }
        toast.error(
          `验证订阅失败（兼容模式失败）: ${formatErrorMessage(fallbackErr)}`,
        );
        return;
      }
    }
