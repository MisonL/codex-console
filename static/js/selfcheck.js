...
function normalizeStatus(status) {
  ...
  if (text === "completed" || text === "pass")
    return { cls: "completed", text: "通过" };
...
function renderRuntime(runtime) {
  ...
  if (!statusNode || !nextRunNode || !lastRunNode || !summaryNode || !logsNode)
    return;

  statusNode.textContent = normalizeStatus(runtime?.last_status).text || "-";
  nextRunNode.textContent = runtime?.next_run_at
    ? format.date(runtime.next_run_at)
    : "-";
  lastRunNode.textContent = runtime?.last_finished_at
    ? format.date(runtime.last_finished_at)
    : "-";
  summaryNode.textContent =
    runtime?.last_run?.summary || runtime?.last_error || "-";

  const logs = Array.isArray(runtime?.logs) ? runtime.logs : [];
  if (!logs.length) {
    logsNode.innerHTML =
      '<div class="selfcheck-log-item"><span>-</span><span>-</span><span>暂无调度日志</span></div>';
    return;
  }

  logsNode.innerHTML = logs
    .map((item) => {
...
function renderRuns() {
  ...
  if (!rows.length) {
    body.innerHTML =
      '<tr><td colspan="9"><div class="empty-state">暂无运行记录</div></td></tr>';
    return;
  }
  ...
  body.innerHTML = rows
    .map((run) => {
      const isSelected =
        Number(run.id) === Number(selfcheckState.selectedRunId);
      return `
...
function renderRunDetail(run) {
  ...
  listNode.innerHTML = checks
    .map((check) => {
      const fixes = Array.isArray(check?.fixes) ? check.fixes : [];
      const detailsText = check?.details
...
async function loadRepairCatalog() {
  try {
    const data = await api.get("/selfcheck/repairs", {
      requestKey: "selfcheck-repairs",
      cancelPrevious: true,
    });
...
async function loadScheduleAndRuntime() {
  const data = await api.get("/selfcheck/schedule", {
    requestKey: "selfcheck-schedule",
    cancelPrevious: true,
...
async function loadRuns() {
  const data = await api.get("/selfcheck/runs?limit=60", {
    requestKey: "selfcheck-runs",
    cancelPrevious: true,
  });
...
async function loadRunDetail(runId) {
  ...
  const run = await api.get(`/selfcheck/runs/${Number(runId)}`, {
    requestKey: `selfcheck-run-${runId}`,
    cancelPrevious: true,
  });
...
async function executeRepair(runId, repairKey, btn) {
  ...
  try {
    const data = await api.post(
      `/selfcheck/runs/${Number(runId)}/repairs/${encodeURIComponent(repairKey)}`,
      {},
    );
...
function collectCheckedPreviewKeys() {
  const nodes = Array.from(
    document.querySelectorAll(".repair-center-key:checked"),
  );
  return nodes
    .map((node) => String(node?.dataset?.repairKey || "").trim())
    .filter(Boolean);
}
...
async function rollbackRepairPoint(rollbackId, btn) {
  ...
  try {
    const data = await api.post(
      `/selfcheck/repair-center/rollbacks/${encodeURIComponent(id)}/rollback`,
      {},
    );
    toast.success(
      `回滚完成：恢复账号 ${Number(data?.result?.restored_accounts || 0)} 条`,
    );
    await loadRuns();
    await loadRepairRollbacks();
  } catch (error) {
...
function bindEvents() {
  ...
  const saveScheduleBtn = document.getElementById(
    "selfcheck-save-schedule-btn",
  );
  ...
  const repairRollbackRefreshBtn = document.getElementById(
    "repair-center-refresh-rollbacks-btn",
  );
...
function startPolling() {
  ...
  selfcheckState.pollTimer = setInterval(async () => {
    try {
      await loadScheduleAndRuntime();
      const running = selfcheckState.runs.some((run) =>
        ["running", "pending"].includes(String(run.status || "").toLowerCase()),
      );
      if (running) {
        await loadRuns();
      }
