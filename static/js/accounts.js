...
function normalizeTaskState(taskRef = {}) {
  const status = String(taskRef?.status || "")
    .trim()
    .toLowerCase();
...
function getRunningBatchTasks() {
  return Object.entries(activeBatchTasks)
    ...
    .filter(
      (task) =>
        task.id &&
        !TASK_TERMINAL_STATUSES.has(String(task.status || "").toLowerCase()),
    );
}
...
function patchCodexAuthTask(key, patch = {}) {
  ...
  if (!current) return;
  activeCodexAuthTasks[key] = normalizeTaskState({
    ...current,
    ...(patch || {}),
  });
  updateBatchButtons();
...
async function watchDomainTask(
  fetchTask,
  onUpdate,
...
async function watchCodexAuthTask(
  taskId,
  onUpdate,
...
function collectValidatedStatusMap(taskOrResult) {
  const detailRows = Array.isArray(taskOrResult?.details)
    ...
    : Array.isArray(taskOrResult?.result?.details)
      ? taskOrResult.result.details
      : [];
...
const elements = {
  ...
  autoQuickRefreshSettingsBtn: document.getElementById(
    "auto-quick-refresh-settings-btn",
  ),
  ...
  autoQuickRefreshEnabled: document.getElementById(
    "auto-quick-refresh-enabled",
  ),
  autoQuickRefreshInterval: document.getElementById(
    "auto-quick-refresh-interval",
  ),
  ...
  autoQuickRefreshRuntime: document.getElementById(
    "auto-quick-refresh-runtime",
  ),
  closeAutoQuickRefreshModalBtn: document.getElementById(
    "close-auto-quick-refresh-modal",
  ),
  cancelAutoQuickRefreshBtn: document.getElementById(
    "cancel-auto-quick-refresh-btn",
  ),
  saveAutoQuickRefreshBtn: document.getElementById(
    "save-auto-quick-refresh-btn",
  ),
  codexAuthWorkbenchBtn: document.getElementById("codex-auth-workbench-btn"),
  codexAuthWorkbenchModal: document.getElementById(
    "codex-auth-workbench-modal",
  ),
  closeCodexAuthWorkbenchModalBtn: document.getElementById(
    "close-codex-auth-workbench-modal",
  ),
  codexAuthSelectionCount: document.getElementById(
    "codex-auth-selection-count",
  ),
  ...
  cancelCodexAuthModalBtn: document.getElementById(
    "cancel-codex-auth-modal-btn",
  ),
  confirmCodexAuthModalBtn: document.getElementById(
    "confirm-codex-auth-modal-btn",
  ),
};
...
function initEventListeners() {
  ...
  elements.autoQuickRefreshSettingsBtn?.addEventListener(
    "click",
    openAutoQuickRefreshModal,
  );
  ...
  elements.codexAuthWorkbenchBtn?.addEventListener(
    "click",
    openCodexAuthWorkbenchModal,
  );
  elements.codexAuthAuditBtn?.addEventListener("click", () =>
    openCodexAuthModal("audit"),
  );
  elements.codexAuthRepairBtn?.addEventListener("click", () =>
    openCodexAuthModal("repair"),
  );
  elements.codexAuthGenerateBtn?.addEventListener("click", () =>
    openCodexAuthModal("generate"),
  );
  elements.codexAuthExportBtn?.addEventListener("click", () =>
    openCodexAuthModal("export"),
  );
  ...
  elements.closeAutoQuickRefreshModalBtn?.addEventListener(
    "click",
    closeAutoQuickRefreshModal,
  );
  elements.cancelAutoQuickRefreshBtn?.addEventListener(
    "click",
    closeAutoQuickRefreshModal,
  );
  elements.saveAutoQuickRefreshBtn?.addEventListener(
    "click",
  ...
  elements.closeCodexAuthWorkbenchModalBtn?.addEventListener(
    "click",
  ...
  elements.closeCodexAuthModalBtn?.addEventListener(
    "click",
    closeCodexAuthModal,
  );
  elements.cancelCodexAuthModalBtn?.addEventListener(
    "click",
    closeCodexAuthModal,
  );
  elements.confirmCodexAuthModalBtn?.addEventListener(
    "click",
    executePendingCodexAuthAction,
  );
...
function renderAutoQuickRefreshRuntime(runtime) {
  ...
  const rows = logs
    ...
    .reverse()
    .map((item) => {
      const levelRaw = String(item?.level || "info")
        .trim()
        .toLowerCase();
      const level = ["success", "warning", "error", "info"].includes(levelRaw)
        ? levelRaw
        : "info";
...
function fillAutoQuickRefreshForm(options = {}) {
  ...
  const modalActive =
    elements.autoQuickRefreshModal?.classList.contains("active");
  if (!force && modalActive && autoQuickRefreshFormDirty) {
    syncSettings = false;
  }

  if (syncSettings && elements.autoQuickRefreshEnabled) {
    elements.autoQuickRefreshEnabled.checked = Boolean(
      autoQuickRefreshSettings.enabled,
    );
  }
  if (syncSettings && elements.autoQuickRefreshInterval) {
    elements.autoQuickRefreshInterval.value = String(
      autoQuickRefreshSettings.interval_minutes || 30,
    );
  }
  if (syncSettings && elements.autoQuickRefreshRetry) {
    elements.autoQuickRefreshRetry.value = String(
      autoQuickRefreshSettings.retry_limit || 2,
    );
  }
  ...
  if (syncRuntime && elements.autoQuickRefreshRuntime) {
    elements.autoQuickRefreshRuntime.innerHTML = renderAutoQuickRefreshRuntime(
      autoQuickRefreshSettings.runtime || {},
    );
  }
}
...
async function saveAutoQuickRefreshSettings() {
  ...
  const interval = Math.max(
    5,
    Math.min(1440, Number(elements.autoQuickRefreshInterval?.value || 30)),
  );
  const retryLimit = Math.max(
    0,
    Math.min(5, Number(elements.autoQuickRefreshRetry?.value || 2)),
  );
  ...
  try {
    ...
    toast.success(
      runNow ? "设置已保存，已触发一次立即执行" : "定时自动一键刷新设置已保存",
    );
    autoQuickRefreshFormDirty = false;
    closeAutoQuickRefreshModal();
...
async function loadStats() {
  try {
    ...
    const parentCount = Number(
      data.tagged_role_counts?.parent ?? data.by_role_tag?.parent ?? 0,
    );
    const childCount = Number(
      data.tagged_role_counts?.child ?? data.by_role_tag?.child ?? 0,
    );
...
function renderSubscriptionStatus(subscriptionType) {
  const normalized = normalizeSubscriptionType(subscriptionType);
  const variant =
    normalized === "plus" || normalized === "team" ? normalized : "free";
  const label = variant.toUpperCase();
  const title =
    variant === "free" ? "未检测到 Plus/Team 订阅" : `已订阅: ${variant}`;
  return `
...
function renderCodexAuthState(codexAuth) {
  const health =
    String(codexAuth?.health || "unknown")
      .trim()
      .toLowerCase() || "unknown";
...
async function pauseActiveBatchTasks() {
  ...
  try {
    const results = await Promise.allSettled(
      tasks.map((task) =>
        api.post(
          `/tasks/${task.domain}/${task.id}/pause`,
          {},
          {
            timeoutMs: 15000,
            retry: 0,
...
async function resumeActiveBatchTasks() {
  ...
  try {
    const results = await Promise.allSettled(
      tasks.map((task) =>
        api.post(
          `/tasks/${task.domain}/${task.id}/resume`,
          {},
          {
            timeoutMs: 15000,
            retry: 0,
...
function openCodexAuthModal(action) {
  ...
  const definitions = {
    audit: {
      ...
      action: "批量审计",
      purpose:
        "用严格 Codex Auth 链路判断账号当前是否可修、是否被 add-phone 门控拦截。",
    ...
    repair: {
      ...
      action: "批量修复",
      purpose:
        "对残缺账号执行严格 Codex Auth 登录，成功后补齐 refresh_token、id_token 并生成标准 auth.json。",
...
async function executePendingCodexAuthAction() {
  ...
  try {
    if (action === "audit") {
      ...
      await runCodexAuthAsyncTask(
        "repair",
        "/accounts/codex-auth/repair/async",
      );
    } else if (action === "generate") {
      await runCodexAuthAsyncTask(
        "generate",
        "/accounts/codex-auth/generate/async",
      );
...
async function runCodexAuthAsyncTask(key, endpoint) {
  ...
  try {
    ...
    if (status === "completed") {
      toast.success(
        `${state[2]}完成：成功 ${result.success_count || 0}，失败 ${result.failed_count || 0}`,
      );
    } else if (status === "cancelled") {
      toast.warning(`${state[2]}已取消`);
    } else {
      toast.error(
        `${state[2]}失败: ${finalTask?.error || finalTask?.message || "未知错误"}`,
      );
    }
...
function updateBatchButtons() {
  const count = getEffectiveCount();
  const baseDisabled =
    count === 0 || isQuickWorkflowRunning || isTaskPausing || isTaskResuming;
  syncCodexAuthWorkbenchState();
  ...
  elements.batchCheckSubBtn.disabled =
    baseDisabled || isBatchCheckingSubscription;
  ...
  if (elements.codexAuthAuditBtn) {
    elements.codexAuthAuditBtn.disabled =
      count === 0 ||
      isCodexAuthAuditing ||
      isCodexAuthRepairing ||
      isCodexAuthGenerating ||
      isCodexAuthExporting;
    elements.codexAuthAuditBtn.textContent = isCodexAuthAuditing
      ? "审计中..."
      : "开始审计";
  }
  if (elements.codexAuthRepairBtn) {
    elements.codexAuthRepairBtn.disabled =
      count === 0 ||
      isCodexAuthAuditing ||
      isCodexAuthRepairing ||
      isCodexAuthGenerating ||
      isCodexAuthExporting;
    elements.codexAuthRepairBtn.textContent = isCodexAuthRepairing
      ? "修复中..."
      : "开始修复";
  }
  if (elements.codexAuthGenerateBtn) {
    elements.codexAuthGenerateBtn.disabled =
      count === 0 ||
      isCodexAuthAuditing ||
      isCodexAuthRepairing ||
      isCodexAuthGenerating ||
      isCodexAuthExporting;
    elements.codexAuthGenerateBtn.textContent = isCodexAuthGenerating
      ? "生成中..."
      : "开始生成";
  }
  if (elements.codexAuthExportBtn) {
    elements.codexAuthExportBtn.disabled =
      count === 0 ||
      isCodexAuthAuditing ||
      isCodexAuthRepairing ||
      isCodexAuthGenerating ||
      isCodexAuthExporting;
    elements.codexAuthExportBtn.textContent = isCodexAuthExporting
      ? "导出中..."
      : "开始导出";
  }
  ...
  elements.batchDeleteBtn.textContent =
    count > 0 ? `删除 (${count})` : "批量删除";
  elements.batchRefreshBtn.textContent = "刷新 Token";
  elements.batchValidateBtn.textContent = "验证 Token";
  ...
  try {
    ...
    const finalTask = await watchAccountTask(taskId, (progressTask) => {
      ...
      if (elements.quickRefreshBtn && !isQuickWorkflowRunning) {
        elements.quickRefreshBtn.textContent = paused
          ? `已暂停 ${completed}/${total}`
          : `刷新中 ${completed}/${total}`;
      }
      if (onProgress) {
    ...
    if (status === "completed") {
      if (showToast) {
        toast.success(
          `成功刷新 ${result.success_count || 0} 个，失败 ${result.failed_count || 0} 个`,
        );
      }
    } else if (status === "cancelled") {
      if (showToast) {
        toast.warning(
          `任务已取消（成功 ${result.success_count || 0}，失败 ${result.failed_count || 0}）`,
          5000,
        );
      }
    } else {
      if (showToast) {
        toast.error(
          `任务执行失败: ${finalTask?.error || finalTask?.message || "未知错误"}`,
        );
      }
    }
    if (reloadAfter) {
    ...
    return {
      ...
      error:
        status === "failed"
          ? finalTask?.error || finalTask?.message || "未知错误"
          : null,
    };
  } catch (error) {
    if (showToast) {
...
async function runBatchValidateTask(payload, count, sourceLabel, options = {}) {
  ...
  try {
    ...
    if (status === "completed") {
      ...
      if (showToast) {
        toast.warning(
          `任务已取消（有效 ${result.valid_count || 0}，无效 ${result.invalid_count || 0}）`,
          5000,
        );
      }
    } else if (showToast) {
...
async function runBatchCheckSubscriptionTask(
  payload,
  count,
  sourceLabel,
  options = {},
  ...
  try {
    const task = await api.post(
      "/payment/accounts/batch-check-subscription/async",
      payload,
      {
        timeoutMs: 20000,
        retry: 0,
...
async function runOverviewRefreshTask(
  payload,
  count,
  sourceLabel,
  options = {},
  ...
  try {
    ...
    if (status === "completed") {
      if (showToast) {
        toast.success(
          `总览刷新完成：成功 ${result.success_count || 0}，失败 ${result.failed_count || 0}`,
        );
      }
    } else if (status === "cancelled") {
      if (showToast) {
            ...
            <div class="info-grid">
                        ...
                        ${
                          account.password
                            ? `<code style="font-size: 0.75rem;">${escapeHtml(account.password)}</code>
                               <button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(account.password)}')" title="复制">CP</button>`
                            : "-"
                        }
                    </span>
                </div>
                ...
                <div class="info-item" style="grid-column: span 2;">
                    ...
                    <div class="value" style="font-size: 0.7rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
                        ...
                        ${tokens.access_token ? `<button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(tokens.access_token)}')" style="margin-left: 8px;">CP</button>` : ""}
                    </div>
                </div>
                <div class="info-item" style="grid-column: span 2;">
                    ...
                    <div class="value" style="font-size: 0.7rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
                        ...
                        ${tokens.refresh_token ? `<button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(tokens.refresh_token)}')" style="margin-left: 8px;">CP</button>` : ""}
                    </div>
                </div>
                <div class="info-item" style="grid-column: span 2;">
                    ...
                    <div class="value" style="font-size: 0.7rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
                        ...
                        ${
                          tokens.session_token || account.session_token
                            ? `<button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(tokens.session_token || account.session_token)}')" style="margin-left: 8px;">CP</button>`
                            : ""
                        }
                ...
                <div class="info-item" style="grid-column: span 2;">
                    ...
                    <div class="value" style="font-size: 0.75rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
                        ...
                        ${tokens.device_id || account.device_id ? `<button class="btn btn-ghost btn-sm" onclick="copyToClipboard('${escapeHtml(tokens.device_id || account.device_id)}')" style="margin-left: 8px;">CP</button>` : ""}
                    </div>
                </div>
                <div class="info-item" style="grid-column: span 2;">
...
function selectCpaService() {
  return new Promise(async (resolve) => {
    ...
    if (services.length === 0) {
      listEl.innerHTML =
        '<div style="text-align:center;color:var(--text-muted);padding:12px;">暂无已启用的 CPA 服务，将使用全局配置</div>';
    } else {
...
function normalizeAccountLabel(value) {
  const text = String(value || "")
    .trim()
    .toLowerCase();
  if (text === "mother" || text === "parent" || text === "manager")
    return "mother";
...
async function markAccountLabel(id, currentLabel = "none") {
  ...
  const defaultValue =
    current === "mother" ? "mother" : current === "child" ? "child" : "none";
  const input = prompt(
    "请输入账号标号（mother=母号 / child=子号 / none=普通）:",
    defaultValue,
  );
...
async function handleBatchCheckSubscription() {
  ...
  const confirmed = await confirm(
    `确定要检测选中的 ${count} 个账号的订阅状态吗？`,
  );
  if (!confirmed) return;
  await runBatchCheckSubscriptionTask(
    buildBatchPayload(),
    count,
    "批量检测订阅",
  );
...
function selectSub2ApiService() {
  return new Promise(async (resolve) => {
    ...
    if (services.length === 0) {
      listEl.innerHTML =
        '<div style="text-align:center;color:var(--text-muted);padding:12px;">暂无已启用的 Sub2API 服务，将自动选择第一个</div>';
    } else {
      listEl.innerHTML = services
        .map(
          (s) => `
...
async function handleBatchUploadSub2Api() {
  ...
  const confirmed = await confirm(
    `确定要将选中的 ${count} 个账号上传到 Sub2API 吗？`,
  );
  if (!confirmed) return;
...
async function uploadToSub2Api(id) {
  ...
  try {
    ...
    if (result.success) {
      ...
      toast.error(
        "上传失败: " + (result.error || result.message || "未知错误"),
      );
    }
  } catch (e) {
...
function selectTmService() {
  return new Promise(async (resolve) => {
    ...
    if (services.length === 0) {
      listEl.innerHTML =
        '<div style="text-align:center;color:var(--text-muted);padding:12px;">暂无已启用的 Team Manager 服务，将自动选择第一个</div>';
    } else {
      listEl.innerHTML = services
        .map(
          (s) => `
...
async function handleBatchUploadTm() {
  ...
  const confirmed = await confirm(
    `确定要将选中的 ${count} 个账号上传到 Team Manager 吗？`,
  );
  if (!confirmed) return;
