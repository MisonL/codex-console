...
const elements = {
  ...
  emailServiceGroup: document
    .getElementById("email-service")
    ?.closest(".form-group"),
  regMode: document.getElementById("reg-mode"),
  registrationType: document.getElementById("registration-type"),
  refreshTokenEnabled: document.getElementById("refresh-token-enabled"),
  ...
  outlookAccountsContainer: document.getElementById(
    "outlook-accounts-container",
  ),
  ...
  outlookWaitStrategyHint: document.getElementById(
    "outlook-wait-strategy-hint",
  ),
  ...
  sub2apiServiceSelectGroup: document.getElementById(
    "sub2api-service-select-group",
  ),
  ...
  newApiServiceSelectGroup: document.getElementById(
    "new-api-service-select-group",
  ),
  ...
  autoRegistrationCheckInterval: document.getElementById(
    "auto-registration-check-interval",
  ),
  autoRegistrationMinReady: document.getElementById(
    "auto-registration-min-ready",
  ),
  autoRegistrationCpaServiceId: document.getElementById(
    "auto-registration-cpa-service-id",
  ),
  autoRegistrationEmailServiceType: document.getElementById(
    "auto-registration-email-service-type",
  ),
  autoRegistrationEmailServiceId: document.getElementById(
    "auto-registration-email-service-id",
  ),
  ...
  autoRegistrationConcurrency: document.getElementById(
    "auto-registration-concurrency",
  ),
  autoRegistrationIntervalGroup: document.getElementById(
    "auto-registration-interval-group",
  ),
  autoRegistrationIntervalMin: document.getElementById(
    "auto-registration-interval-min",
  ),
  autoRegistrationIntervalMax: document.getElementById(
    "auto-registration-interval-max",
  ),
};
...
async function initAutoUploadOptions() {
  await Promise.all([
    loadServiceSelect(
      "/cpa-services?enabled=true",
      elements.cpaServiceSelect,
      elements.autoUploadCpa,
      elements.cpaServiceSelectGroup,
    ),
    loadServiceSelect(
      "/sub2api-services?enabled=true",
      elements.sub2apiServiceSelect,
      elements.autoUploadSub2api,
      elements.sub2apiServiceSelectGroup,
    ),
    loadServiceSelect(
      "/tm-services?enabled=true",
      elements.tmServiceSelect,
      elements.autoUploadTm,
      elements.tmServiceSelectGroup,
    ),
    loadServiceSelect(
      "/new-api-services?enabled=true",
      elements.newApiServiceSelect,
      elements.autoUploadNewApi,
      elements.newApiServiceSelectGroup,
    ),
  ]);
}
...
function updateMsdLabel(ddId) {
  ...
  if (checked.length === 0) label.textContent = "未选择";
    ...
    label.textContent = `全部 (${all.length})`;
  else
...
function getSelectedServiceIds(container) {
  if (!container) return [];
  return Array.from(container.querySelectorAll(".msd-item input:checked")).map(
...
function initEventListeners() {
  ...
  if (elements.autoRegistrationEmailServiceType) {
    elements.autoRegistrationEmailServiceType.addEventListener("change", () =>
      populateAutoRegistrationEmailServiceOptions(0),
    );
  }
  ...
  elements.clearLogBtn.addEventListener("click", () => {
    elements.consoleLog.innerHTML =
      '<div class="log-line info">[系统] 日志已清空</div>';
  ...
  if (elements.refreshSchedulesBtn) {
    elements.refreshSchedulesBtn.addEventListener("click", () =>
      loadScheduledJobs(),
    );
  }
}
...
async function loadAvailableServices() {
  try {
    ...
    populateAutoRegistrationEmailServiceOptions(
      parseInt(elements.autoRegistrationEmailServiceId?.value || "0", 10) || 0,
    );

    if (editingScheduleJobUuid) {
      const currentJob = scheduledJobs.find(
        (item) => item.job_uuid === editingScheduleJobUuid,
      );
      if (currentJob) {
...
function updateEmailServiceOptions() {
  ...
  // 官方临时邮箱渠道
  if (
    (availableServices.tempmail && availableServices.tempmail.available) ||
    ...
    if (availableServices.yyds_mail && availableServices.yyds_mail.available) {
      availableServices.yyds_mail.services.forEach((service) => {
        ...
        option.textContent =
          service.name +
          (service.default_domain ? ` (@${service.default_domain})` : "");
  ...
  if (availableServices.temp_mail && availableServices.temp_mail.available) {
    ...
    availableServices.temp_mail.services.forEach((service) => {
      ...
      option.textContent =
        service.name + (service.domain ? ` (@${service.domain})` : "");
  ...
  if (availableServices.cloudmail && availableServices.cloudmail.available) {
    const optgroup = document.createElement("optgroup");
    optgroup.label = `CloudMail (${availableServices.cloudmail.count} 个服务)`;
  ...
  if (availableServices.luckmail && availableServices.luckmail.available) {
    ...
    availableServices.luckmail.services.forEach((service) => {
      ...
      option.textContent =
        service.name +
        (service.preferred_domain ? ` (@${service.preferred_domain})` : "");
...
function handleServiceChange(e) {
  ...
  if (type === "outlook") {
    ...
    const service = availableServices.yyds_mail.services.find(
      (s) => (s.id || "default") == id,
    );
    if (service) {
    ...
    const service = availableServices.temp_mail.services.find(
      (s) => s.id == id,
    );
    if (service) {
    ...
    const service = availableServices.cloudmail.services.find(
      (s) => s.id == id,
    );
    if (service) {
    ...
    const service = availableServices.duck_mail.services.find(
      (s) => s.id == id,
    );
    if (service) {
    ...
    const service = availableServices.imap_mail.services.find(
      (s) => s.id == id,
    );
    if (service) {
...
function handleModeChange(e) {
  ...
  if (elements.autoRegistrationSection) {
    elements.autoRegistrationSection.style.display = isAutoMode
      ? "block"
      : "none";
  }
...
function initScheduleForm() {
  ...
  elements.scheduleTriggerType.addEventListener(
    "change",
...
function updateScheduleTriggerFields() {
  ...
  elements.scheduleIntervalFields.style.display =
    triggerType === "interval" ? "block" : "none";
  elements.scheduleTimepointFields.style.display =
    triggerType === "timepoint" ? "block" : "none";
}
...
function buildCurrentRegistrationConfig() {
  ...
  const baseConfig = {
    ...
    registration_type: elements.registrationType
      ? elements.registrationType.value
      : "child",
    refresh_token_enabled: elements.refreshTokenEnabled
      ? elements.refreshTokenEnabled.checked
      : true,
    reg_mode: isOutlookBatchMode
      ? "outlook_batch"
      : isBatchMode
        ? "batch"
        : "single",
    auto_upload_cpa: elements.autoUploadCpa
      ? elements.autoUploadCpa.checked
      : false,
    cpa_service_ids:
      elements.autoUploadCpa && elements.autoUploadCpa.checked
        ? getSelectedServiceIds(elements.cpaServiceSelect)
        : [],
    auto_upload_sub2api: elements.autoUploadSub2api
      ? elements.autoUploadSub2api.checked
      : false,
    sub2api_service_ids:
      elements.autoUploadSub2api && elements.autoUploadSub2api.checked
        ? getSelectedServiceIds(elements.sub2apiServiceSelect)
        : [],
    auto_upload_tm: elements.autoUploadTm
      ? elements.autoUploadTm.checked
      : false,
    tm_service_ids:
      elements.autoUploadTm && elements.autoUploadTm.checked
        ? getSelectedServiceIds(elements.tmServiceSelect)
        : [],
    auto_upload_new_api: elements.autoUploadNewApi
      ? elements.autoUploadNewApi.checked
      : false,
    new_api_service_ids:
      elements.autoUploadNewApi && elements.autoUploadNewApi.checked
        ? getSelectedServiceIds(elements.newApiServiceSelect)
        : [],
  };
  ...
  if (isOutlookBatchMode) {
    const selectedIds = [];
    document
      .querySelectorAll(".outlook-account-checkbox:checked")
      .forEach((cb) => {
    ...
    return {
      ...
      concurrency: Math.min(
        50,
        Math.max(1, parseInt(elements.outlookConcurrencyCount.value) || 3),
      ),
  ...
  if (isBatchMode) {
    return {
      ...
      concurrency: Math.min(
        50,
        Math.max(1, parseInt(elements.concurrencyCount.value) || 1),
      ),
...
async function handleScheduleSubmit(e) {
  ...
  try {
    ...
    if (
      registrationConfig.email_service_type === "outlook_batch" &&
      (!registrationConfig.service_ids ||
        registrationConfig.service_ids.length === 0)
    ) {
...
function setRegistrationConfigToForm(config) {
  ...
  const serviceValue =
    emailServiceType === "outlook_batch"
      ? "outlook_batch:all"
  ...
  elements.cpaServiceSelectGroup.style.display = elements.autoUploadCpa.checked
    ? "block"
    : "none";
  elements.autoUploadSub2api.checked = !!registrationConfig.auto_upload_sub2api;
  elements.sub2apiServiceSelectGroup.style.display = elements.autoUploadSub2api
    .checked
    ? "block"
    : "none";
  elements.autoUploadTm.checked = !!registrationConfig.auto_upload_tm;
  elements.tmServiceSelectGroup.style.display = elements.autoUploadTm.checked
    ? "block"
    : "none";
  if (elements.autoUploadNewApi) {
    elements.autoUploadNewApi.checked =
      !!registrationConfig.auto_upload_new_api;
  }
  if (elements.newApiServiceSelectGroup && elements.autoUploadNewApi) {
    elements.newApiServiceSelectGroup.style.display = elements.autoUploadNewApi
      .checked
      ? "block"
      : "none";
  }
  ...
  setSelectedServiceIds(
    elements.cpaServiceSelect,
    registrationConfig.cpa_service_ids || [],
  );
  setSelectedServiceIds(
    elements.sub2apiServiceSelect,
    registrationConfig.sub2api_service_ids || [],
  );
  setSelectedServiceIds(
    elements.tmServiceSelect,
    registrationConfig.tm_service_ids || [],
  );
  setSelectedServiceIds(
    elements.newApiServiceSelect,
    registrationConfig.new_api_service_ids || [],
  );
  ...
  if (emailServiceType === "outlook_batch") {
    ...
    elements.batchOptions.style.display = "none";
    if (
      Array.isArray(registrationConfig.service_ids) &&
      registrationConfig.service_ids.length
    ) {
      const selectedSet = new Set(
        registrationConfig.service_ids.map((item) => parseInt(item)),
      );
      document.querySelectorAll(".outlook-account-checkbox").forEach((cb) => {
        cb.checked = selectedSet.has(parseInt(cb.value));
      });
    }
    elements.outlookSkipRegistered.checked =
      registrationConfig.skip_registered !== false;
    ...
    elements.outlookConcurrencyCount.value =
      registrationConfig.concurrency || 3;
    elements.outlookConcurrencyMode.value =
      registrationConfig.mode || "pipeline";
    handleConcurrencyModeChange(
      elements.outlookConcurrencyMode,
      elements.outlookConcurrencyHint,
      elements.outlookIntervalGroup,
  ...
  elements.outlookBatchSection.style.display = "none";
  elements.regModeGroup.style.display = "block";
  elements.regMode.value =
    registrationConfig.reg_mode === "batch" ? "batch" : "single";
  handleModeChange({ target: elements.regMode });
...
function setSelectedServiceIds(container, selectedIds) {
  if (!container) return;
  const selectedSet = new Set(
    (selectedIds || []).map((item) => parseInt(item)),
  );
  container.querySelectorAll(".msd-item input").forEach((cb) => {
    cb.checked =
      selectedSet.size === 0 ? true : selectedSet.has(parseInt(cb.value));
  });
...
function renderScheduledJobs() {
  ...
  elements.scheduleJobsTable.innerHTML = scheduledJobs
    .map(
      (job) => `
        <tr>
            <td>
...
async function editScheduledJob(jobUuid) {
  try {
    ...
    if (job.schedule_type === "interval") {
      elements.scheduleIntervalMinutes.value =
        job.schedule_config?.interval_minutes || 60;
    } else {
      elements.scheduleEveryNDays.value =
        job.schedule_config?.every_n_days || 1;
      elements.scheduleTimeOfDay.value =
        job.schedule_config?.time_of_day || "09:00";
      elements.scheduleStartDate.value =
        job.schedule_config?.start_date ||
...
async function toggleScheduledJob(jobUuid, shouldEnable) {
  try {
    const endpoint = shouldEnable
      ? `/registration/schedules/${jobUuid}/enable`
      : `/registration/schedules/${jobUuid}/pause`;
...
async function handleStartRegistration(e) {
  ...
  if (isAutoMode) {
    await loading.withButton(
      elements.startBtn,
      handleSaveAutoRegistration(),
      "保存中...",
    );
    return;
  }
  ...
  // 禁用开始按钮
  loading.show(elements.startBtn, "提交中...");
  elements.cancelBtn.disabled = false;
...
async function loadAutoRegistrationSettings() {
  ...
  try {
    ...
    elements.autoRegistrationCheckInterval.value =
      reg.auto_check_interval || 60;
    elements.autoRegistrationMinReady.value =
      reg.auto_min_ready_auth_files || 1;
    elements.autoRegistrationEmailServiceType.value =
      reg.auto_email_service_type || "tempmail";
    ...
    updateRegistrationWaitStrategyHints();
    elements.autoRegistrationEmailServiceId.dataset.selectedId = String(
      reg.auto_email_service_id || 0,
    );
    elements.autoRegistrationCpaServiceId.dataset.selectedId = String(
      reg.auto_cpa_service_id || 0,
    );
...
async function loadAutoRegistrationCpaOptions() {
  ...
  try {
    ...
    services.forEach((service) => {
      options.push(
        `<option value="${service.id}">${escapeHtml(service.name)} (#${service.id})</option>`,
      );
    });
    elements.autoRegistrationCpaServiceId.innerHTML = options.join("");
    elements.autoRegistrationCpaServiceId.value =
      elements.autoRegistrationCpaServiceId.dataset.selectedId || "0";
  } catch (error) {
...
function populateAutoRegistrationEmailServiceOptions(selectedId = 0) {
  if (
    !elements.autoRegistrationEmailServiceId ||
    !elements.autoRegistrationEmailServiceType
  )
    return;
  const selectedType =
    elements.autoRegistrationEmailServiceType.value || "tempmail";
  ...
  elements.autoRegistrationEmailServiceId.value = String(
    selectedId ||
      elements.autoRegistrationEmailServiceId.dataset.selectedId ||
      0,
  );
}

async function handleSaveAutoRegistration() {
  const autoCheckInterval =
    parseInt(elements.autoRegistrationCheckInterval.value, 10) || 60;
  const autoMinReady =
    parseInt(elements.autoRegistrationMinReady.value, 10) || 1;
  const autoEmailServiceId =
    parseInt(elements.autoRegistrationEmailServiceId.value, 10) || 0;
  const autoConcurrency =
    parseInt(elements.autoRegistrationConcurrency.value, 10) || 1;
  const autoIntervalMin =
    parseInt(elements.autoRegistrationIntervalMin.value, 10) || 0;
  const autoIntervalMax =
    parseInt(elements.autoRegistrationIntervalMax.value, 10) || 0;
  const autoCpaServiceId =
    parseInt(elements.autoRegistrationCpaServiceId.value, 10) || 0;
...
async function handleSingleRegistration(requestData) {
  ...
  try {
    ...
    transitionStartButtonToRunning();
    // 持久化到 sessionStorage，跨页面导航后可恢复
    sessionStorage.setItem(
      "activeTask",
      JSON.stringify({ task_uuid: data.task_uuid, mode: "single" }),
    );
...
function getReconnectDelay(attempt) {
  return Math.min(
    WS_RECONNECT_BASE_DELAY * 2 ** Math.max(0, attempt - 1),
    WS_RECONNECT_MAX_DELAY,
  );
}

function clearWebSocketReconnect() {
  if (webSocket?.clearReconnect) {
    webSocket.clearReconnect();
  }
  if (wsReconnectTimer) {
...
function clearBatchWebSocketReconnect() {
  if (batchWebSocket?.clearReconnect) {
    batchWebSocket.clearReconnect();
  }
...
function scheduleWebSocketReconnect(taskUuid) {
  if (!taskUuid || !webSocket?.scheduleReconnect) return;
  webSocket.scheduleReconnect({ code: 1006 });
}

function scheduleBatchWebSocketReconnect(batchId) {
  if (!batchId || !batchWebSocket?.scheduleReconnect) return;
  batchWebSocket.scheduleReconnect({ code: 1006 });
}
...
function startCurrentBatchPolling(batchId) {
  ...
  const pollingMode =
    currentBatch && currentBatch.batch_id === batchId
...
function connectWebSocket(taskUuid) {
  ...
  if (
    webSocket?.taskUuid === taskUuid &&
    [WebSocket.OPEN, WebSocket.CONNECTING].includes(webSocket.readyState)
  ) {
    return;
  }

  if (webSocket && webSocket.taskUuid !== taskUuid) {
    webSocket.disconnect({ manual: true, reason: "switch_task" });
  }
  ...
  webSocket = createManagedWebSocket({
    label: "task",
    url: () => wsUrl,
    heartbeatPayload: { type: "ping" },
    heartbeatIntervalMs: 25000,
    baseDelayMs: WS_RECONNECT_BASE_DELAY,
    maxDelayMs: WS_RECONNECT_MAX_DELAY,
    shouldReconnect: () =>
      !wsManualClose &&
      !taskCompleted &&
      taskFinalStatus === null &&
    ...
    onMessage: (event) => {
      ...
      if (data.type === "log") {
        ...
        if (data.email_service) {
          elements.taskService.textContent = getServiceTypeText(
            data.email_service,
          );
        }
        ...
        // 检查是否完成
        if (
          ["completed", "failed", "cancelled", "cancelling"].includes(
            data.status,
          )
        ) {
    ...
    onReconnectSchedule: ({ attempt, delay }) => {
      useWebSocket = false;
      startLogPolling(taskUuid);
      wsReconnectAttempts = attempt;
      addLog(
        "warning",
        `[系统] 任务监控连接已断开，${Math.round(delay / 100) / 10} 秒后尝试重连`,
      );
    },
  });
...
function disconnectWebSocket() {
  ...
  clearWebSocketReconnect();
  if (webSocket) {
    webSocket.disconnect({ manual: true, reason: "task_closed" });
    webSocket = null;
  }
}
...
function stopWebSocketHeartbeat() {
  if (webSocket?.clearHeartbeat) {
    webSocket.clearHeartbeat();
  }
    ...
    try {
      ...
      if (data.finished) {
        ...
        if (!toastShown) {
          ...
          addLog(
            "info",
            `[完成] 批量任务完成！成功: ${data.success}, 失败: ${data.failed}`,
          );
          if (data.success > 0) {
...
function transitionStartButtonToRunning() {
...
function updateAutoMonitorHeader(status, lastCheckedAt) {
  if (!elements.autoMonitorStatusBadge || !elements.autoMonitorLastChecked)
    return;
...
async function pollAutoRegistrationStatus() {
  try {
    ...
    if (elements.taskLastChecked) {
      elements.taskLastChecked.textContent = formatAutoMonitorTimestamp(
    ...
    const effectiveStatus =
      data.batch && data.batch.cancelled && !data.batch.finished
        ? "cancelling"
        : data.status || "pending";
    ...
    if (data.batch) {
      ...
      if (
        (!batchWebSocket || batchWebSocket.readyState === WebSocket.CLOSED) &&
        !data.batch.finished
      ) {
...
async function loadRecentAccounts() {
  try {
    ...
    elements.recentAccountsTable.innerHTML = data.accounts
      .map(
        (account) => `
            <tr data-id="${account.id}">
                ...
                <td>
                    <span style="display:inline-flex;align-items:center;gap:4px;">
                        ...
                        <button class="btn-copy-icon copy-email-btn" data-email="${escapeHtml(account.email)}" title="复制邮箱">CP</button>
                    </span>
                </td>
                    ...
                    ${
                      account.password
                        ? `<span style="display:inline-flex;align-items:center;gap:4px;">
                            ...
                            <button class="btn-copy-icon copy-pwd-btn" data-pwd="${escapeHtml(account.password)}" title="复制密码">CP</button>
                           </span>`
                        : "-"
                    }
                </td>
    ...
    // 绑定复制按钮事件
    elements.recentAccountsTable
      .querySelectorAll(".copy-email-btn")
      .forEach((btn) => {
...
function renderTodayStats(total, success, failed, rate) {
  ...
  if (elements.todayStatsRate) {
    ...
    elements.todayStatsRate.classList.remove(
      "rate-high",
      "rate-mid",
      "rate-low",
    );
...
function getLogType(log) {
  ...
  if (
    ...
    lowerLog.includes("成功") ||
    lowerLog.includes("完成")
  ) {
    return "success";
  }
  return "info";
...
async function loadOutlookAccounts() {
  try {
    elements.outlookAccountsContainer.innerHTML =
      '<div class="loading-placeholder" style="text-align: center; padding: var(--spacing-md); color: var(--text-muted);">加载中...</div>';
    ...
    addLog(
      "info",
      `[系统] 已加载 ${data.total} 个 Outlook 账户 (已注册: ${data.registered_count}, 未注册: ${data.unregistered_count})`,
    );
  } catch (error) {
...
function renderOutlookAccountsList() {
  if (outlookAccounts.length === 0) {
    elements.outlookAccountsContainer.innerHTML =
      '<div style="text-align: center; padding: var(--spacing-md); color: var(--text-muted);">没有可用的 Outlook 账户</div>';
    return;
  }

  const html = outlookAccounts
    .map(
      (account) => `
...
async function handleOutlookBatchRegistration() {
  ...
  addLog(
    "info",
    `[系统] 正在启动 Outlook 批量注册 (${selectedIds.length} 个账户)...`,
  );
  ...
  try {
    ...
    addLog(
      "info",
      `[系统] 总数: ${data.total}, 跳过已注册: ${data.skipped}, 待注册: ${data.to_register}`,
    );
...
function connectBatchWebSocket(batchId) {
  ...
  if (
    batchWebSocket?.batchId === batchId &&
    [WebSocket.OPEN, WebSocket.CONNECTING].includes(batchWebSocket.readyState)
  ) {
    return;
  }

  if (batchWebSocket && batchWebSocket.batchId !== batchId) {
    batchWebSocket.disconnect({ manual: true, reason: "switch_batch" });
  }
  ...
  batchWebSocket = createManagedWebSocket({
    label: "batch",
    url: () => wsUrl,
    heartbeatPayload: { type: "ping" },
    heartbeatIntervalMs: 25000,
        ...
        if (
          ...
          if (!toastShown) {
            ...
            if (data.status === "completed") {
              addLog(
                "success",
                `[完成] 批量任务完成，成功: ${data.success}, 失败: ${data.failed}, 跳过: ${data.skipped || 0}`,
              );
              if (data.success > 0) {
    ...
    onReconnectSchedule: ({ attempt, delay }) => {
      batchWsReconnectAttempts = attempt;
      startCurrentBatchPolling(batchId);
      addLog(
        "warning",
        `[系统] 批量任务监控连接已断开，${Math.round(delay / 100) / 10} 秒后尝试重连`,
      );
    },
  });

  batchWebSocket.batchId = batchId;
  batchWebSocket.connect();
}
...
function disconnectBatchWebSocket() {
  ...
  clearBatchWebSocketReconnect();
  if (batchWebSocket) {
    batchWebSocket.disconnect({ manual: true, reason: "batch_closed" });
    batchWebSocket = null;
  }
}
...
function stopBatchWebSocketHeartbeat() {
  if (batchWebSocket?.clearHeartbeat) {
    batchWebSocket.clearHeartbeat();
  }
...
function cancelBatchViaWebSocket() {
  batchWebSocket?.send({ type: "cancel" });
}
    ...
    try {
      ...
      if (data.finished) {
        ...
        if (!toastShown) {
          ...
          addLog(
            "info",
            `[完成] Outlook 批量任务完成！成功: ${data.success}, 失败: ${data.failed}, 跳过: ${data.skipped || 0}`,
          );
          if (data.success > 0) {
            toast.success(`Outlook 批量注册完成，成功 ${data.success} 个`);
            loadRecentAccounts();
...
async function restoreActiveTask() {
  ...
  if (mode === "single" && task_uuid) {
    ...
    try {
      ...
      addLog(
        "info",
        `[系统] 检测到进行中的任务，正在重连监控... (${task_uuid.substring(0, 8)})`,
      );
    ...
    try {
      ...
      addLog(
        "info",
        `[系统] 检测到进行中的批量任务，正在重连监控... (${batch_id.substring(0, 8)})`,
      );
...
async function refreshOutlookRegistrationStatus() {
  try {
    ...
    const data = await api.post("/registration/outlook/check-accounts", {
      service_ids: ids,
    });
    outlookAccounts = data.accounts || [];
