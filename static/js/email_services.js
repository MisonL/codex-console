...
const elements = {
  ...
  addTempmailBuiltinFields: document.getElementById(
    "add-tempmail-builtin-fields",
  ),
  ...
  editTempmailBuiltinFields: document.getElementById(
    "edit-tempmail-builtin-fields",
  ),
...
const CUSTOM_SUBTYPE_LABELS = {
  ...
  cloudmail: "CloudMail（Cloudflare Workers 邮箱）",
...
function initializeEmailServiceModals() {
  const register = window.modal?.register?.bind(window.modal);
  if (!register) return;
  [
    elements.addCustomModal,
    elements.editCustomModal,
    elements.editOutlookModal,
  ]
    .filter(Boolean)
...
function initEventListeners() {
  ...
  elements.addCustomBtn.addEventListener("click", () => {
    ...
    openModal(elements.addCustomModal);
  });
  elements.closeCustomModal.addEventListener("click", () =>
    closeModal(elements.addCustomModal),
  );
  ...
  elements.closeEditCustomModal.addEventListener("click", () =>
    closeModal(elements.editCustomModal),
  );
  ...
  elements.closeEditOutlookModal.addEventListener("click", () =>
    closeModal(elements.editOutlookModal),
  );
...
function switchAddSubType(subType) {
  ...
  elements.addTempmailBuiltinFields.style.display =
    subType === "tempmail_builtin" ? "" : "none";
  ...
  elements.addLuckmailFields.style.display =
    subType === "luckmail" ? "" : "none";
  elements.addFreemailFields.style.display =
    subType === "freemail" ? "" : "none";
  elements.addCloudmailFields.style.display =
    subType === "cloudmail" ? "" : "none";
...
function switchEditSubType(subType) {
  ...
  elements.editTempmailBuiltinFields.style.display =
    subType === "tempmail_builtin" ? "" : "none";
  ...
  elements.editDuckmailFields.style.display =
    subType === "duckmail" ? "" : "none";
  elements.editLuckmailFields.style.display =
    subType === "luckmail" ? "" : "none";
  elements.editFreemailFields.style.display =
    subType === "freemail" ? "" : "none";
  elements.editCloudmailFields.style.display =
    subType === "cloudmail" ? "" : "none";
            ...
            <tr data-id="${service.id}">
                ...
                <td>
                    <div style="display:flex;gap:4px;align-items:center;white-space:nowrap;">
                        <button type="button" class="btn btn-secondary btn-sm" onclick="editOutlookService(${service.id})">编辑</button>
                        <div class="dropdown" style="position:relative;">
                            <button type="button" class="btn btn-secondary btn-sm" onclick="event.stopPropagation();toggleEmailMoreMenu(this)">更多</button>
                        ...
                        <button type="button" class="btn btn-danger btn-sm" onclick="deleteService(${service.id}, '${escapeHtml(service.name)}')">删除</button>
                    </div>
                </td>
            </tr>
        `,
    ...
    .map((service) => {
      return `
        <tr data-id="${service.id}">
            ...
            <td>
                <div style="display:flex;gap:4px;align-items:center;white-space:nowrap;">
                    <button type="button" class="btn btn-secondary btn-sm" onclick="editCustomService(${service.id}, '${service._subType}')">编辑</button>
                    <div class="dropdown" style="position:relative;">
                        <button type="button" class="btn btn-secondary btn-sm" onclick="event.stopPropagation();toggleEmailMoreMenu(this)">更多</button>
                        <div class="dropdown-menu" style="min-width:80px;">
                            <a href="#" class="dropdown-item" onclick="event.preventDefault();closeEmailMoreMenu(this);toggleService(${service.id}, ${!service.enabled})">${service.enabled ? "禁用" : "启用"}</a>
...
async function loadTempmailConfig() {
  try {
    ...
    if (settings.yyds_mail) {
      ...
      elements.yydsDefaultDomain.value =
        settings.yyds_mail.default_domain || "";
      elements.yydsEnabled.checked = settings.yyds_mail.enabled === true;
      elements.yydsApiKey.value = "";
      elements.yydsApiKey.placeholder = settings.yyds_mail.has_api_key
        ? "已设置，留空保持不变"
        : "请输入 YYDS Mail API Key";
    }
  } catch (error) {
    // 忽略错误
  }
...
async function handleBatchDeleteOutlook() {
  ...
  const confirmed = await confirm(
    `确定要删除选中的 ${selectedOutlook.size} 个账户吗？`,
  );
  if (!confirmed) return;
  try {
...
async function handleTestYyds() {
  ...
  try {
    ...
    if (result.success) toast.success("YYDS Mail 连接正常");
      ...
      toast.error(
        "连接失败: " + (result.error || result.message || "未知错误"),
      );
  } catch (error) {
    toast.error("测试失败: " + error.message);
  } finally {
...
async function editCustomService(id, subType) {
  try {
    ...
    if (resolvedSubType === "moemail") {
      ...
      document.getElementById("edit-tpo-base-url").value =
        service.config?.base_url || "";
      document.getElementById("edit-tpo-timeout").value =
        service.config?.timeout || 30;
      document.getElementById("edit-tpo-max-retries").value =
        service.config?.max_retries || 3;
    } else if (resolvedSubType === "yyds_mail") {
      document.getElementById("edit-yyds-base-url").value =
        service.config?.base_url || "";
      document.getElementById("edit-yyds-api-key").value = "";
      document.getElementById("edit-yyds-api-key").placeholder = service.config
        ?.api_key
        ? "已设置，留空保持不变"
        : "请输入 API Key";
      document.getElementById("edit-yyds-default-domain").value =
        service.config?.default_domain || "";
      document.getElementById("edit-yyds-timeout").value =
        service.config?.timeout || 30;
      document.getElementById("edit-yyds-max-retries").value =
        service.config?.max_retries || 3;
      ...
      document.getElementById("edit-lm-base-url").value =
        service.config?.base_url || "";
      document.getElementById("edit-lm-api-key").value = "";
      document.getElementById("edit-lm-api-key").placeholder = service.config
        ?.api_key
        ? "已设置，留空保持不变"
        : "请输入 API Key";
      document.getElementById("edit-lm-project-code").value =
        service.config?.project_code || "openai";
      document.getElementById("edit-lm-email-type").value =
        service.config?.email_type || "ms_graph";
      document.getElementById("edit-lm-preferred-domain").value =
        service.config?.preferred_domain || "";
      ...
      document.getElementById("edit-cm-base-url").value =
        service.config?.base_url || "";
      document.getElementById("edit-cm-admin-email").value =
        service.config?.admin_email || "";
      document.getElementById("edit-cm-admin-password").value = "";
      document.getElementById("edit-cm-admin-password").placeholder = service
        .config?.admin_password
        ? "已设置，留空保持不变"
        : "请输入管理员密码";
      // 处理域名：如果是数组，转换为逗号分隔的字符串
      const domain = service.config?.domain;
      const domainStr = Array.isArray(domain)
        ? domain.join(", ")
        : domain || "";
      document.getElementById("edit-cm-domain").value = domainStr;
      // 设置子域
      document.getElementById("edit-cm-subdomain").value =
        service.config?.subdomain || "";
    } else {
    ...
    openModal(elements.editCustomModal);
  } catch (error) {
...
async function editOutlookService(id) {
  try {
    ...
    openModal(elements.editOutlookModal);
  } catch (error) {
    toast.error("获取服务信息失败: " + error.message);
  }
}
